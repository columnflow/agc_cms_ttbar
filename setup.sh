...
setup_agc() {
    ...
    #   - "AGC_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "AGC_" variables is documented below.
    #
    ...
    #   AGC_SRC_BASE
    #       The absolute directory of the AGC source repository from which configuration files
    #       are read.

    # prevent repeated setups
    if [ "${AGC_SETUP}" = "1" ]; then
        >&2 echo "the cf_agc analysis was already succesfully setup"
    ...
    #
    # global variables
    # (AGC = cf_agc, CF = columnflow)
    #

    # start exporting variables
    export AGC_BASE="${this_dir}"
    export CF_BASE="${this_dir}/modules/columnflow"
    export CF_REPO_BASE="${AGC_BASE}"
    export CF_SETUP_NAME="${setup_name}"
    export CF_REPO_BASE_ALIAS="AGC_BASE"
    export AGC_SRC_BASE="${AGC_BASE}/modules/analysis-grand-challenge"
    ...
    if [ "${CF_REMOTE_JOB}" != "1" ]; then
        cf_setup_interactive_body() {
            ...
            query CF_DATA "Local data directory" "\$AGC_BASE/data" "./data"
            query CF_STORE_NAME "Relative path used in store paths (see next queries)" "agc_store"
            ...
            query AGC_BUNDLE_CMSSW "Install and bundle CMSSW sandboxes for job submission?" "True"
        }
        cf_setup_interactive "${CF_SETUP_NAME}" "${AGC_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${AGC_BASE}/bin:${PATH}"
    export PYTHONPATH="${AGC_BASE}:${AGC_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${AGC_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${AGC_BASE}/modules" ); do
            cf_init_submodule "${AGC_BASE}" "modules/${m}"
        done
    fi
    ...
    export LAW_HOME="${AGC_BASE}/.law"
    export LAW_CONFIG_FILE="${AGC_BASE}/law.cfg"
...
main() {
    ...
    if setup_agc "$@"; then
        cf_color green "cf_agc analysis successfully setup"
        return "0"
    else
        local code="$?"
